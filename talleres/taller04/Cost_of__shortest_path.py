...
def cost_of_shortestAUX(graph,source:int,dest:int,visited:list, shortest_cost = math.inf)->int:
  print("Source",source)
  print("Dest",dest)
  visited[source] = True
  if source == dest:
    ...
    for nextVert in graph.getSuccessors(source):
      if not visited[nextVert[0]]:
        cost_of_shortest_past_my_nextVert = int(graph.getWeight(source,nextVert)) + int(cost_of_shortestAUX(graph,nextVert[0],dest,visited))
...
print(graph.getWeight(0,1)) #4
print("Shortest path")
#print(cost_of_shortest(graph,size,0,0))#0
print(cost_of_shortest(graph,size,0,1))#4
